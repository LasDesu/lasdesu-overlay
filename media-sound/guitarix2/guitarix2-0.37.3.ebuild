...
IUSE="avahi +capture debug faust ffmpeg ladspa lv2 +meterbridge nls"
...
RDEPEND="dev-cpp/eigen:3
	...
	!ffmpeg? ( media-libs/zita-convolver )
...
src_configure() {
	local mywafconfargs=(
		...
		$(usex ladspa --ladspadir="${EPREFIX}/usr/$(get_libdir)/ladspa --ladspa" "--no-ladspa --no-new-ladspa")
		$(usex lv2 --lv2dir="${EPREFIX}/usr/$(get_libdir)/lv2" --no-lv2)
		$(usex ffmpeg --convolver-ffmpeg "")
	)
